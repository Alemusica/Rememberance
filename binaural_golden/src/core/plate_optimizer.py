...
class OptimizationConfig:
    ...
    # Constraints - LUTHERIE MODE
    # ═══════════════════════════════════════════════════════════════════════════
    # In lutherie, cutouts (sound holes, f-holes) are used to TUNE the plate:
    #   - Violin f-holes: tune Helmholtz resonance ~280Hz
    #   - Guitar rosette: tune air modes
    #   - Bass bar: redistribute modes
    #
    ...
    max_cutouts: int = 4  # Abilitati per accordatura (liuteria)
    min_cutout_radius: float = 0.02  # Min 20mm (lavorabile con fresa)
    max_cutout_radius: float = 0.08  # Max 80mm (non indebolisce troppo)
    cutout_min_edge_distance: float = 0.05  # Min 50mm dal bordo
    min_material_fraction: float = 0.80  # 80% materiale (permette cutouts)
    enforce_convexity: bool = True  # Forma esterna convessa
...
# NOTA: generate_water_molecule_shape, generate_butterfly_shape, generate_lemniscate_shape
# sono stati RIMOSSI - forme non realizzabili per produzione.
# Vedi audit commit be90aca per dettagli.
