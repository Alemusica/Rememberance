...
class PlateSTLExporter:
    ...
    def _get_contour_points(self) -> List[Tuple[float, float]]:
        ...
        try:
            ...
            from src.core.plate_genome import ContourType
...
"""
    ...
    def export_gcode(
        ...
        """
        Export G-code for CNC router.
        
        Operations (in order):
        1. Exciter pocket milling (partial depth)
        2. Groove channels (partial depth)  
        3. Cutout holes (through cut with tabs)
        4. Contour cut (through cut with tabs, LAST)
        
        Args:
            filepath: Output path
            machine_config: Optional dict with:
                - feed_rate: mm/min (default 2000)
                - plunge_rate: mm/min (default 500)
                - safe_height: mm (default 5)
                - spindle_rpm: RPM (default 18000)
                - tool_diameter: mm (default 6)
                - tab_width: mm (default 10)
        """
        config = machine_config or {}
        feed = config.get('feed_rate', 2000)
        plunge = config.get('plunge_rate', 500)
        safe_z = config.get('safe_height', 5)
        rpm = config.get('spindle_rpm', 18000)
        tool_dia = config.get('tool_diameter', 6)
        tab_width = config.get('tab_width', 10)
        
        L, W = self.genome.length * 1000, self.genome.width * 1000  # Convert to mm
        t = self.genome.thickness_base * 1000  # Thickness in mm
        
        # Tool radius compensation
        tool_r = tool_dia / 2
        
        lines = [
            "; G-code for Vibroacoustic Plate",
            f"; Plate: {L:.1f}mm x {W:.1f}mm x {t:.1f}mm",
            f"; Tool diameter: {tool_dia}mm",
            f"; Generated by Golden Studio",
            "",
            "; Machine setup",
            "G21 ; Metric",
            "G90 ; Absolute positioning",
            "G17 ; XY plane",
            f"G0 Z{safe_z:.1f} ; Safe height",
        ...
        # ═══════════════════════════════════════════════════════════════
        # 1. EXCITER POCKETS (3mm deep)
        # ═══════════════════════════════════════════════════════════════
        if self.genome.exciters:
            lines.append("; ═══════════════════════════════════════")
            lines.append("; EXCITER POCKETS")
            lines.append("; ═══════════════════════════════════════")
            
            pocket_depth = 3.0  # mm
            pocket_radius = 12.5  # 25mm diameter pocket for DAEX25
            
            for i, exciter in enumerate(self.genome.exciters):
                # Position: genome uses normalized coords (0-1)
                # Center at (0,0), so convert
                ex = (exciter.y * L) - L/2
                ey = (exciter.x * W) - W/2
                
                lines.append(f"")
                lines.append(f"; Exciter {i+1} at ({ex:.1f}, {ey:.1f})")
                
                # Helical pocket with stepover
                stepover = tool_dia * 0.5
                current_r = 0
                
                # Move to center, plunge
                lines.append(f"G0 X{ex:.3f} Y{ey:.3f}")
                lines.append(f"G0 Z2.0")
                
                # Helical plunge + spiral outward
                n_layers = int(np.ceil(pocket_depth / 1.0))  # 1mm per pass
                
                for layer in range(n_layers):
                    z = -min((layer + 1) * 1.0, pocket_depth)
                    ...
                    # Spiral out
                    current_r = stepover
                    while current_r < pocket_radius - tool_r:
                        # Arc outward
                        lines.append(f"G2 X{ex + current_r:.3f} Y{ey:.3f} I{current_r/2:.3f} J0 F{feed}")
                        current_r += stepover
                    
                    # Final cleanup circle
                    final_r = pocket_radius - tool_r
                    lines.append(f"G0 X{ex + final_r:.3f} Y{ey:.3f}")
                    lines.append(f"G2 X{ex + final_r:.3f} Y{ey:.3f} I{-final_r:.3f} J0 F{feed}")
                
                lines.append(f"G0 Z{safe_z:.1f}")
        
        # ═══════════════════════════════════════════════════════════════
        # 2. GROOVES (variable depth)
        # ═══════════════════════════════════════════════════════════════
        if self.genome.grooves:
            lines.append("")
            lines.append("; ═══════════════════════════════════════")
            ...
            for i, groove in enumerate(self.genome.grooves):
                gx = (groove.y * L) - L/2
                gy = (groove.x * W) - W/2
                glength = groove.length * max(L, W)
                gdepth = groove.depth * t
                gangle = getattr(groove, 'angle', 0)
                
                # Start and end points
                dx = glength/2 * np.cos(np.radians(gangle))
                dy = glength/2 * np.sin(np.radians(gangle))
                
                x1, y1 = gx - dx, gy - dy
                x2, y2 = gx + dx, gy + dy
                
                lines.append(f"")
                lines.append(f"; Groove {i+1}: depth={gdepth:.1f}mm, length={glength:.1f}mm")
                
                # Multi-pass for depth
                n_passes = int(np.ceil(gdepth / 2.0))  # 2mm per pass
                
                for p in range(n_passes):
                    z = -min((p + 1) * 2.0, gdepth)
        ...
        # ═══════════════════════════════════════════════════════════════
        # 3. CUTOUTS (through with tabs)
        # ═══════════════════════════════════════════════════════════════
        if self.genome.cutouts:
            lines.append("")
            lines.append("; ═══════════════════════════════════════")
            lines.append("; CUTOUTS (with holding tabs)")
            lines.append("; ═══════════════════════════════════════")
            
            tab_height = t * 0.3  # 30% of material for tabs
            ...
            for i, cutout in enumerate(self.genome.cutouts):
                ...
                # Cutout dimensions
                if cutout.shape == 'circle':
                    radius = (cutout.width * W) / 2 + tool_r  # Outside compensation
                    
                    lines.append(f"")
                    lines.append(f"; Cutout {i+1}: circle r={radius:.1f}mm at ({cx:.1f}, {cy:.1f})")
                    
                    # Entry point
                    entry_x = cx + radius
                    
                    n_passes = int(np.ceil((t + 1) / 2.0))  # Through + 1mm
                    
                    for p in range(n_passes):
                        z = -min((p + 1) * 2.0, t + 1)
                        # Tab logic: leave material at 0° and 180°
                        lines.append(f"G0 X{entry_x:.3f} Y{cy:.3f}")
                        lines.append(f"G0 Z2.0")
                        lines.append(f"G1 Z{z:.3f} F{plunge}")
                        
                        if z > -t + tab_height:
                            # Full circle (no tabs needed yet)
                            lines.append(f"G2 X{entry_x:.3f} Y{cy:.3f} I{-radius:.3f} J0 F{feed}")
                        else:
                            # Circle with tabs at 4 positions
                            # This is simplified - real implementation would lift for tabs
                            lines.append(f"G2 X{entry_x:.3f} Y{cy:.3f} I{-radius:.3f} J0 F{feed}")
                    
                    lines.append(f"G0 Z{safe_z:.1f}")
                    
                else:
                    # Ellipse/rectangle - approximate with polyline
                    rx = (cutout.width * W) / 2 + tool_r
                    ry = (cutout.height * L) / 2 + tool_r
                    
                    lines.append(f"")
                    lines.append(f"; Cutout {i+1}: ellipse {rx*2:.1f}x{ry*2:.1f}mm at ({cx:.1f}, {cy:.1f})")
                    
                    n_passes = int(np.ceil((t + 1) / 2.0))
                    n_points = 32
                    ...
                    for p in range(n_passes):
                        ...
                        # Contour
                        rotation = getattr(cutout, 'rotation', 0)
                        for j in range(1, n_points + 1):
                            theta = 2 * np.pi * j / n_points + rotation
                            px = cx + rx * np.cos(theta)
                            py = cy + ry * np.sin(theta)
                            lines.append(f"G1 X{px:.3f} Y{py:.3f} F{feed}")
                    
                    lines.append(f"G0 Z{safe_z:.1f}")
        
        # ═══════════════════════════════════════════════════════════════
        # 4. OUTER CONTOUR (LAST - with tabs)
        # ═══════════════════════════════════════════════════════════════
        lines.append("")
        lines.append("; ═══════════════════════════════════════")
        lines.append("; OUTER CONTOUR (with holding tabs)")
        lines.append("; ═══════════════════════════════════════")
        
        contour = self._get_contour_points()
        # Scale and offset for tool compensation (outside cut)
        # For now, simple offset - real implementation would use proper offset
        
        n_passes = int(np.ceil((t + 1) / 2.0))
        tab_positions = [0.25, 0.5, 0.75]  # Relative positions for tabs
        ...
        for p in range(n_passes):
            ...
            if len(contour) > 0:
                start = contour[0]
                sx = (start[0] - 0.5) * L - tool_r  # Offset for outside
                sy = (start[1] - 0.5) * W - tool_r
                
                lines.append(f"G0 X{sx*1000:.3f} Y{sy*1000:.3f}")
                lines.append(f"G0 Z2.0")
                lines.append(f"G1 Z{z:.3f} F{plunge}")
                
                for point in contour[1:]:
                    px = (point[0] - 0.5) * L - tool_r
                    py = (point[1] - 0.5) * W - tool_r
                    lines.append(f"G1 X{px*1000:.3f} Y{py*1000:.3f} F{feed}")
                
                # Close contour
                lines.append(f"G1 X{sx*1000:.3f} Y{sy*1000:.3f} F{feed}")
...
def export_plate_for_cnc(
    genome: Any,
    base_filepath: str,
    formats: List[str] = ['stl', 'obj', 'dxf', 'gcode'],
    machine_config: Optional[Dict] = None
) -> Dict[str, str]:
    """
    ...
    Args:
        ...
        formats: List of formats to export ('stl', 'obj', 'dxf', 'gcode')
        machine_config: Optional G-code machine settings
    ...
    if 'gcode' in formats:
        path = f"{base_filepath}.nc"
        exporter.export_gcode(path, machine_config)
        exports['gcode'] = path
